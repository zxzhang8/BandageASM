...
namespace
{
bool splitGfaTag(const QString &field, QString * tagName, QString * value)
{
    int firstColon = field.indexOf(':');
    int secondColon = field.indexOf(':', firstColon + 1);
    if (firstColon <= 0 || secondColon < 0)
        return false;

    if (tagName != 0)
        *tagName = field.left(firstColon);
    if (value != 0)
        *value = field.mid(secondColon + 1);
    return true;
}
}
...
QStringList DeBruijnNode::getGfaTagNames() const
{
    QStringList names;
    for (int i = 0; i < m_gfaExtraSegmentTags.size(); ++i)
    {
        QString name;
        if (splitGfaTag(m_gfaExtraSegmentTags[i], &name, 0) && !names.contains(name))
            names << name;
    }
    return names;
}
...
QString DeBruijnNode::getGfaTagValue(const QString &tagName) const
{
    for (int i = 0; i < m_gfaExtraSegmentTags.size(); ++i)
    {
        QString name;
        QString value;
        if (splitGfaTag(m_gfaExtraSegmentTags[i], &name, &value) && name == tagName)
            return value;
    }

    // Reverse-complement nodes generated by Bandage do not carry their own
    // copy of the segment tags.  Those tags describe the same GFA segment.
    if (m_gfaExtraSegmentTags.isEmpty() && m_reverseComplement != 0)
    {
        const QStringList reverseTags = m_reverseComplement->getGfaExtraSegmentTags();
        for (int i = 0; i < reverseTags.size(); ++i)
        {
            QString name;
            QString value;
            if (splitGfaTag(reverseTags[i], &name, &value) && name == tagName)
                return value;
        }
    }
